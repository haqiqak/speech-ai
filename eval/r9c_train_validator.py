...
MODEL_OUT = EVAL / "r9c_validator_model"
...
# Resume from the latest checkpoint if this run was paused earlier (same
# dataset/split/config -- only the seed differs from R9B, and resuming
# preserves optimizer/scheduler state exactly, no re-run needed for
# already-completed epochs).
ckpt_dir = MODEL_OUT / "checkpoints"
existing_ckpts = sorted(ckpt_dir.glob("checkpoint-*"), key=lambda p: int(p.name.split("-")[1])) if ckpt_dir.exists() else []
