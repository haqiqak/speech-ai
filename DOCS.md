...
| `PROBLEM_FORMULATION.md` | Stage 4A design record for the persistent speaker difficulty profile: problem formulation, schema, ARPAbet-vs-IPA and JSON-vs-SQLite research, the text-entry/flagging interaction design (and why a JS-based one was researched but deliberately not built), rejected alternatives, testing results. | When the profile schema or its interaction design changes | New (2026-08-15). Uses `RESEARCH.md`'s same finding/interpretation/hypothesis/limitation/recommendation legend. Read before touching `difficulty_profile.py` or the Speaker Difficulty Profile UI panel. |

## Live application path (the app depends on these; keep lean)

| File | Purpose | Notes |
|---|---|---|
| `app.py` | Streamlit UI + orchestration: auth gate → sanitize → rewrite → render. | Largest single file; mixes UI, session-state management, and light business logic (e.g. `_risk_chips`, `_rebuild_sentence`). Candidate for splitting if it keeps growing — not a finding, an observation. As of Stage 4A, also renders the "Speaker Difficulty Profile" panel (`_difficulty_profile()`/`_render_difficulty_category()`), which is UI/data-plumbing on top of `difficulty_profile.py`, not part of the reformulation logic itself. |
| `difficulty_profile.py` | Stage 4A: `DifficultyProfile` — persistent, user-declared sounds/words/phrases, explicitly independent categories, `meta`-extensible entries. | New (2026-08-15). See `PROBLEM_FORMULATION.md` for full design rationale. Deliberately separate from `profiling/profile.py`'s learned `SpeakerDifficultyProfile` — reconciling them is `ROADMAP.md` R12, not yet done. |
| `auth.py` | Login/register screen; calls into `user_store.py`; gates all of `app.py` via `require_auth()` + `st.stop()`. | |
| `user_store.py` | File-based per-user JSON store; thin public API intentionally designed to be swappable for SQLite/Postgres. | SHA-256 password hashing — see `DECISION_LOG.md` 2026-06-07-A. As of Stage 4A also stores `difficulty_profile` (via `load_difficulty_profile()`/`save_difficulty_profile()`), which auto-derives the legacy `phoneme_profile.stutter_patterns`/`.blocked_words` mirror on every save — see `DECISION_LOG.md` 2026-08-15-C for why that mirror exists instead of a second independently-edited copy. |
...
| `grammar.py` | `sanitize_input()` (grammar correction, ~10 layers) + `SentenceRewriter` (the core substitution loop: POS-tag → protect → candidates → SBERT rank → phoneme gate → inflect → rebuild). Largest logic file (~1,600 lines). | Unmodified by Stage 4A — verified via `tests/smoke.py` byte-identical output. |
...
| `phonetic.py` | ARPAbet onset extraction (CMU dict + grapheme fallback), the hard-coded `word_difficulty()` heuristic. | Standalone by design (imports only `paths`/nltk/wordfreq); the difficulty formula's weights are un-validated — see `DECISION_LOG.md` and `VALIDATION.md`. Stage 4A added one new, purely additive function, `full_pronunciation()` — CMU-dict-only, informational display use only (`difficulty_profile.py`), no scoring/gating role; deliberately does *not* guess for OOV words (returns `None`) unlike `onset()`'s grapheme fallback. |
...
| `tests/difficulty_profile_test.py` | Stage 4A: 26 tests for `difficulty_profile.py` — add/remove/dedup per category, text edge cases, pronunciation derivation (including OOV), persistence across a simulated restart, legacy-profile migration. | New (2026-08-15). `tests/app_test.py` was also extended (scenario 5) to exercise the actual Streamlit widgets for the same feature, not just the module directly. |
