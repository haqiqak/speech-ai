...
**[FACT] Real human-vs-Claude comparison data: n=8, 7/8 agree (87.5%)**, computed directly from `stage_lr/ingest_real_human_pair.summarize()`, not by hand — every record carries both a real participant's verdict and a same-session Claude verdict on the identical pair, per `ingest_real_human_pair.py`'s hard rule. Now spans **three** distinct real participants (friend_1: 6 records; friend_2: 1 record; friend_3: 1 record, 2026-09-01), not one or two.

**[INTERPRETATION, heavily qualified — n=8 across three participants is not a result to act on, and isn't treated as one]** Cannot statistically distinguish anything on this scale. Worth naming without over-reading it: it's higher than the earlier Claude-vs-Claude ceiling check (68%/81%, `DECISION_LOG.md` 2026-08-30-N) and LR.2's own 5-term model (55.2%/58.8%, 2026-08-30-M) — the opposite direction from what that ceiling check's own caveat anticipated as the risk. Three participants can't confirm or rule out that concern either way. Needs data from more real, distinct participants before this is anything but a small set of points to keep in view.

**[NOT DONE]** No conclusion drawn, no LR.2 weights touched, no change to LR.3's gating. n=8 across three participants is still well within the scale this project's own discipline says not to generalize from.
