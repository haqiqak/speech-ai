...
## Path (b), first real participant — in progress; participant content kept local by design (2026-08-30)

**[FACT] The ask worked — a real answer came back**, and has been processed as far as generating candidates. Per direct instruction, the participant's actual declared profile, sentences, and generated pairs are **not reproduced in this shared document or committed to the repository** — they live only under `stage_lr/data/private/` (gitignored) on the machine handling this. This section records the aggregate, non-identifying facts and one system-level finding, nothing participant-specific.

**[FACT] The mechanism is now generic and reusable**, corrected from an initial version that mistakenly hardcoded the participant's actual words directly in a committed `.py` file — caught before that file was pushed, not after. `stage_lr/harvest_real_profile.py` takes a per-participant input file from `stage_lr/data/private/` and is itself content-free; only `stage_lr/data/private/` and `stage_lr/data/real_human_pairs.json` are gitignored, not the mechanism.

**[FACT] Aggregate result, real numbers, no participant content:** 15 hand-picked sentences run through the real, unmodified `reformulate()` for a real declared profile, for the first time → 5 no substitution change → 5 no second candidate → 5 second candidates found → 0 contaminated → 5 unique. 4 of the 5 "no substitution change" results trace directly to `ROADMAP.md` R13's already-documented gap (declared phrases are stored but never matched against input text) — concrete, non-hypothetical confirmation that this gap has real cost for a real declared profile, not just a theoretical one.

**[FACT, a genuinely new finding, disclosed not fixed — about the system, not the participant] One of the 5 found candidates had to be excluded from what gets shown to the participant.** A real, previously-undocumented defect in `main`'s frozen `sanitize_input()`: it incorrectly applies third-person-singular agreement to an infinitive after "wants to" (a generic English construction, not specific to anything the participant said) — confirmed directly, and confirmed the other 4 sentences are unaffected by checking each one's own `sanitize_input()` output. Doesn't bias the underlying A-vs-B comparison (both candidates inherit the identical pre-existing error), but would have shown a first-time real participant an unexplained, unrelated error — excluded, not silently sent. Not fixed here, per `CLAUDE.md`'s standing instruction for a newly observed failure against the frozen architecture (note it, don't patch it) — logged in `DECISION_LOG.md` 2026-08-30-R.

**[FACT] The participant's picks came back, and the first real human-vs-Claude comparison in this project's Stage LR history now exists.** All 4 pairs processed the same session their picks arrived, per `ingest_real_human_pair.py`'s rule: a Claude verdict obtained on the identical 4 items, blind, via the standard method (`stage_lr/judge_pairs.py`), then both verdicts logged together. **Result: 1/4 agree (25%).**

**[INTERPRETATION, heavily qualified — n=4 is not a result to act on, and isn't treated as one]** This is the first real data point, not a finding. It cannot statistically distinguish anything — n=4 is compatible with almost any true agreement rate. What it *is* worth naming: it's lower than both the earlier Claude-vs-Claude ceiling check (68%/81%, `DECISION_LOG.md` 2026-08-30-N) and LR.2's own 5-term model (55.2%/58.8%, 2026-08-30-M). That's exactly the direction the ceiling check's own disclosed caveat already anticipated — "Claude-consistency, not a true human ceiling" — a real human may simply weigh these three axes differently than Claude does, consistently, not just noisily. One real data point is consistent with that concern; it does not confirm it. Needs far more real pairs, from more than one participant, before this is anything but a flag to keep watching.

**[NOT DONE]** No conclusion drawn, no LR.2 weights touched, no change to LR.3's gating. This is one participant's first 4 answers — exactly the scale this project's own discipline says not to generalize from.

## Path (b), second real participant (2026-08-30)

**[FACT] A second real answer came back, same handling as the first** — real content stays under `stage_lr/data/private/`, gitignored, nothing participant-specific committed. Reusing the now-generic `stage_lr/harvest_real_profile.py` unchanged (no new code needed, exactly as intended).

**[FACT] Aggregate result, real numbers:** 15 hand-picked sentences → 5 no substitution change → 3 no second candidate → **7 second candidates found → 0 contaminated → 7 unique**. A notably higher yield than the first participant's (5 found/5 unique) — no conclusion drawn from that difference at n=2 participants, just reported as-is.

**[FACT, the same defect class, now independently reproduced — strengthens the earlier finding, not a new one]** One of the 7 found candidates again had to be excluded, for the **same underlying `sanitize_input()` defect** disclosed for the first participant (`DECISION_LOG.md` 2026-08-30-R) — this time on a completely different sentence: `"He tried to predict..."` → `"He tried to predicts..."`. Confirmed directly, same method (checked each sentence's own `sanitize_input()` output individually). Two independent real sentences, two different participants, same exact failure shape (an infinitive after "to" incorrectly gets third-person-singular agreement) — this is no longer a single odd case, it's a **reproducible pattern** in `main`'s frozen grammar-correction layer. Still not fixed here — but the case for prioritizing it as routine maintenance (the freeze's own permitted category) is now stronger than when first found. **6 unique clean pairs remain** after this exclusion, ready for the participant to judge.

**[FACT] The second participant's picks came back; processed the same session, same rule.** Claude verdict obtained blind on the identical 6 pairs, both logged together. **Result: 5/6 agree.**

**[FACT] Cumulative real human-vs-Claude data now: n=10, 6/10 agree (60%)**, computed directly from `stage_lr/ingest_real_human_pair.summarize()`, not by hand. Per-participant: participant 1 was 1/4 (25%), participant 2 was 5/6 (83%) — a large spread between two real people.

**[INTERPRETATION, still heavily qualified — n=10 across 2 people is bigger than n=4, but not big]** The spread itself (25% vs. 83%) is the most informative thing here, more than the pooled 60%: it's a concrete illustration of exactly why path (b) needs *multiple* real, distinct people rather than one — a reward model tuned to look good against participant 2 alone could look badly wrong against participant 1, and vice versa. Two participants can't establish which pattern (if either) is representative; that needs more people, which is the same standing ask as before, not a new one. No conclusion drawn, no LR.2 weights touched, no change to LR.3's gating.
