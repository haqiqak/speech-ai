...
**[NOT DONE]** 4 candidate pairs are ready and awaiting the participant's picks. No verdict — human or Claude — has been recorded yet, correctly: per `ingest_real_human_pair.py`'s rule, judging only happens once the participant's picks exist to pair with a same-session Claude verdict.
